...
var clickHandler = {
    ...
    , internalLinkWasClicked: function internalLinkWasClicked(target) {
        ...
        if (this.event.ctrlKey || this.event.shiftKey || this.event.metaKey || // Apple command key
        this.event.button && this.event.button == 1 // Middle click
                ...
                if (win) {
                    // If we were able to create the new tab (eg wasn't blocked by popup blocker)
    ...
    , externalLinkWasClicked: function externalLinkWasClicked(target) {
        this.event.preventDefault(); // Prevent unwanted default behavior. [this] = clickHandler.
        this.event.stopPropagation(); // We're done with the event now
...
var onpageLinkHandler = {
    ...
    linkWasClicked: function linkWasClicked(link) {
    ...
    backButtonWasClicked: function backButtonWasClicked() {
